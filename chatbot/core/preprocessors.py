    ...
    for item in dic.items():
        for i, e in enumerate(x):
            if e.lower() == item[0]:
                ...
                del x[i]
                for ix, token in enumerate(item[1].capitalize().split() if e[0].isupper() else item[1].split() ):
                    x.insert(i+ix,token)
    return x   
...
def corrector(statement):
    ...
    data = replace_from_dict(data, dic = contraction_colloq_dict)

    data = dtknzr.detokenize(data)
    statement.text = data
    return statement